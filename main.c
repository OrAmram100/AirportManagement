...
{ 
	...
	ePrintFlightOrigDest,eSorting,eSearchFlight, eNofOptions
} eMenuOptions;

const char* str[eNofOptions] = { "Add Flight", "Add Airport",
								"PrintCompany", "Print all Airports",
								"Print flights between origin-destination","Sort in ascending order","Search a flight"};
...
int main()
{
	...
	do
	{
		...
		case eSorting:
			getSortTypeFromUser(&company);
			break;
		case eSearchFlight:
			findFlightBySearchMethod(&company);
			break;
		case EXIT:
			printf("Bye bye\n");
			stop = 1;
			saveToFileText(&manager);
