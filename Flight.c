...
int	printFlightToFile(FILE* fp, const Flight* pFlight)
{
	if (fwrite(&pFlight->originCode, sizeof(char), (CODE_LENGTH + 1), fp) != (CODE_LENGTH + 1))
		return 0;
	if (fwrite(&pFlight->destCode, sizeof(char), (CODE_LENGTH + 1), fp) != (CODE_LENGTH + 1))
		return 0;
	if (fwrite(&pFlight->hour, sizeof(int), 1, fp) != 1)
		return 0;
	if (fwrite(&pFlight->date, sizeof(Date), 1, fp) != 1)
		return 0;
	return 1;
}
...
int readFlightFromBFile(FILE* fp, Flight* pFlight)
{
	...
	if (fread(&pFlight->hour, sizeof(int), 1, fp) != 1)
		return 0;
	if (fread(&pFlight->date, sizeof(Date), 1, fp) != 1)
		return 0;
	return 1;
	}
